...
run_ui_test() {
  ...
  local passed_at=0
  local stop_reason=""
  # Anchored on the comma so a prefix cannot pass for a longer number: unanchored, a suite expecting
  # 3 is satisfied by "Executed 30 tests".
  local expected_summary="Executed $expected_count tests\?,"
  ...
  if [[ -z "$stop_reason" ]]; then
    ...
    cat "$log_file"
    # A green suite that ran the wrong number of tests is not a green suite: a test renamed out of
    # the `test` prefix, or skipped at runtime, still reports success for everything that did run.
    if [[ "$status" -eq 0 ]] && ! grep -q "$expected_summary" "$log_file"; then
      ...
      echo "       Either a test stopped running, or the count in UI_SUITES is stale." >&2
      status=1
    fi
    rm -f "$log_file"
...
# The UI suites preflight runs, as expected-test-count:suite. The count catches a suite that
# quietly stops running one of its tests; `require_every_ui_suite_listed` catches the bigger
# version of the same mistake — a whole suite that was never added here. Both are needed: the
# Xcode project is generated by XcodeGen from a directory glob, so a new file under CrumbUITests/
# compiles and is runnable immediately, and nothing except this list decides whether it is ever run.
UI_SUITES=(
  ...
  '4:CrumbUITests/MissionEntryAccessibilityUITests'
  '11:CrumbUITests/MissionsHomeOrderUITests'
  '4:CrumbUITests/MissionKitHeaderUITests'
)

# Fails when a file under CrumbUITests/ defines tests that no entry in UI_SUITES runs. Adding a
# suite and forgetting this list is silent otherwise: preflight stays green while the new coverage
# never executes, which is how MissionsHomeOrderUITests and MissionKitHeaderUITests — 15 tests
# between them — went unrun by every release for two weeks.
require_every_ui_suite_listed() {
  [[ "$DRY_RUN" -eq 1 ]] && return
  local missing=() file suite
  for file in "$ROOT"/CrumbUITests/*.swift; do
    [[ -e "$file" ]] || continue
    grep -qE '^[[:space:]]*(@MainActor[[:space:]]+)?func test' "$file" || continue
    suite="$(basename "$file" .swift)"
    printf '%s\n' "${UI_SUITES[@]}" | grep -q ":CrumbUITests/$suite\$" || missing+=("$suite")
  done
  if (( ${#missing[@]} )); then
    echo "error: these UI suites define tests but preflight never runs them:" >&2
    printf '       %s\n' "${missing[@]}" >&2
    echo "       Add each to UI_SUITES in scripts/testflight.sh as 'count:CrumbUITests/Name'." >&2
    exit 1
  fi
}
